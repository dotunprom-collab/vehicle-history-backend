...
async generatePdfBuffer(
  ...
  return new Promise((resolve, reject) => {
    ...
    // ============================================================
    // FONT LOADING — DM Sans + Instrument Serif
    // ============================================================
    const fontDir = path.join(process.cwd(), 'assets', 'fonts');
    const fSansR = path.join(fontDir, 'DMSans-Regular.ttf');
    const fSansM = path.join(fontDir, 'DMSans-Medium.ttf');
    ...
    // ============================================================
    // STATE — tier flags + data extraction
    // ============================================================
    const isPremium = tier === 'premium';
    const isStandard = tier === 'standard';
    const isFree = tier === 'free';
    ...
    // ============================================================
    // RISK COMPUTATION (Phase 1B logic preserved)
    // ============================================================
    let riskScore = 0;
    const issues: string[] = [];
    const positives: string[] = [];
    ...
    // Hero — serif headline with italic green accent
    text("Your vehicle's", 0, 270, { font: F.serif, size: 38, color: C.text, width: PAGE.width, align: 'center' });
    text('complete history', 0, 312, { font: F.serifIt, size: 44, color: C.green, width: PAGE.width, align: 'center' });

    // Yellow GB plate graphic
    const plateW = 280;
    const plateH = 64;
    const plateX = (PAGE.width - plateW) / 2;
    const plateY = 380;
    ...
    // Vehicle headline (year/make/model)
    const vehicleHeadline = [v.year, v.make, v.model].filter((x) => x && x !== 'Unknown').join(' ');
    if (vehicleHeadline) {
      text(vehicleHeadline, 0, 470, { font: F.sansMed, size: 18, color: C.sub, width: PAGE.width, align: 'center' });
    }
    ...
    // Cover teaser line — what's inside
    const teaserBits: string[] = [];
    teaserBits.push(`${motTests.length} MOT record${motTests.length === 1 ? '' : 's'}`);
    if (isPremium) {
      const keepers = Array.isArray(data?.keeperHistory) ? data.keeperHistory : [];
    ...
    // ============================================================
    // PAGE 2 — AT A GLANCE (Risk gauge + status grid + findings)
    // ============================================================
    doc.addPage();
    drawPageHeader();
    ...
    // Risk gauge card — hero number on left, gauge on right
    const gCardH = 180;
    roundedRect(PAGE.margin, cursorY, PAGE.contentWidth, gCardH, 18, C.card, C.border);
    text('Overall risk assessment', PAGE.margin + 22, cursorY + 20, { font: F.sansMed, size: 11, color: C.sub });

    // Hero risk number — big serif on left
    text(String(riskScore), PAGE.margin + 22, cursorY + 44, { font: F.serif, size: 64, color: riskFg });
    text('/ 100', PAGE.margin + 22 + 80, cursorY + 80, { font: F.sansMed, size: 14, color: C.sub });
    ...
    // Status grid 2x3 — text labels in coloured pills
    const checks = [
      { label: 'Finance', state: isPremium ? (data?.finance === 'outstanding' ? 'warn' : data?.finance === 'clear' ? 'ok' : 'unknown') : 'locked',
        msg: isPremium ? (data?.finance === 'outstanding' ? 'Outstanding' : data?.finance === 'clear' ? 'Clear' : 'Unknown') : 'Premium only' },
      { label: 'Stolen', state: isPremium ? (data?.stolen === 'yes' ? 'warn' : data?.stolen === 'no' ? 'ok' : 'unknown') : 'locked',
    ...
    // Findings — What looks good / Things to check
    if (issues.length > 0 || positives.length > 0) {
      const sumH = Math.max(positives.length, issues.length) * 18 + 50;
      roundedRect(PAGE.margin, cursorY, PAGE.contentWidth, sumH, 14, C.surface);
      const colW = (PAGE.contentWidth - 32) / 2;
    ...
    // ============================================================
    // PAGES 3-7 — STUB PLACEHOLDERS (filled in tomorrow)
    // ============================================================
    const stubs = [
      { num: 3, title: 'Vehicle details', body: 'Full DVLA registration data including make, model, colour, engine, fuel, CO₂, MOT status, tax status. Coming in next deploy.' },
      { num: 4, title: 'MOT history', body: 'Complete DVSA MOT inspection log with pass/fail results, dates, mileages, advisory analysis, and mileage chart. Coming in next deploy.' },
      { num: 5, title: isPremium ? 'Keeper history' : 'Get the full picture', body: isPremium ? 'Full DVLA ownership chain with transfer dates, tenure analysis, and proportional timeline. Coming in next deploy.' : 'Premium upgrade benefits and £3 unlock CTA. Coming in next deploy.' },
      { num: 6, title: 'Detected findings', body: 'AI-derived findings panel with severity tiers (critical / high / minor), buyer recommendations, and cost estimates. Coming in next deploy.' },
      { num: 7, title: 'About this report', body: 'Data sources, accuracy disclaimer, liability notice, and validity window. Coming in next deploy.' },
    ];
    ...
    stubs.forEach((stub) => {
      ...
      drawPageHeader();
      cursorY = 80;
      text(stub.title, PAGE.margin, cursorY, { font: F.serif, size: 32, color: C.text, width: PAGE.contentWidth });
      cursorY += 60;

      // Stub box
      roundedRect(PAGE.margin, cursorY, PAGE.contentWidth, 120, 14, C.surface);
      text('Section under construction', PAGE.margin + 24, cursorY + 24, { font: F.sansMed, size: 13, color: C.amber, width: PAGE.contentWidth - 48 });
      text(stub.body, PAGE.margin + 24, cursorY + 50, { font: F.sans, size: 11, color: C.sub, width: PAGE.contentWidth - 48, lineGap: 3 });

      drawPageFooter(stub.num, 7);
    });
